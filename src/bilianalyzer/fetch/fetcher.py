...
class Fetcher:
    ...
    async def fetch_page_replies(self, bvid: str, index: int) -> list[Reply]:
        page: Page = await get_comments(bvid2aid(bvid), CommentResourceType.VIDEO, index, credential=self.credential)
    ...
    async def fetch_replies(self, bvid: str, limit: int = 20) -> list[Reply]:
        page: Page = await get_comments(bvid2aid(bvid), CommentResourceType.VIDEO, credential=self.credential)
    ...
    def fetch_members(self, replies: Collection[Reply]) -> list[Member]:
        members: list[Member] = []
    ...
    async def fetch_video_info(self, bvid: str) -> list[VideoInfo]:
        video_info: VideoInfo = await Video(bvid, credential=self.credential).get_info()
        return [video_info]
